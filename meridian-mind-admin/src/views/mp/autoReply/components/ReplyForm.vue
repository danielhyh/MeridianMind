<template>
  <div>
    <el-form ref="formRef" :model="replyForm" :rules="rules" label-width="80px">
      <el-form-item v-if="msgType === MsgType.Message" label="消息类型" prop="requestMessageType">
      ...
      <el-form-item v-if="msgType === MsgType.Keyword" label="匹配类型" prop="requestMatch">
        <el-select v-model="replyForm.requestMatch" clearable placeholder="请选择匹配类型">
          <el-option
      ...
      <el-form-item v-if="msgType === MsgType.Keyword" label="关键词" prop="requestKeyword">
        <el-input v-model="replyForm.requestKeyword" clearable placeholder="请输入内容" />
      </el-form-item>
