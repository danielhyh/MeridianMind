<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button :disabled="serialPort || isUse" type="primary" @click="connectToSerialPort">
          连接扫描枪
        </el-button>
        <el-button :disabled="!serialPort || isUse" type="danger" @click="cutPort">
          断开扫描枪
        </el-button>
      </el-form-item>
    </el-form>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      <el-table-column align="center" label="订单号" min-width="180" prop="no" />
      <el-table-column align="center" label="用户信息" min-width="80" prop="user.nickname" />
      <el-table-column
        align="center"
        label="推荐人信息"
        min-width="100"
      ...
      />
      <el-table-column align="center" label="商品信息" min-width="300" prop="spuName">
        <template #default="{ row }">
          <div v-for="item in row.items" :key="item.id" class="flex items-center">
            <el-image
      ...
      <el-table-column
        ...
        prop="payPrice"
      />
      <el-table-column align="center" label="核销员" min-width="70" prop="storeStaffName" />
      <el-table-column align="center" label="核销门店" min-width="80" prop="pickUpStoreId">
        <template #default="{ row }">
          {{ pickUpStoreList.find((p) => p.id === row.pickUpStoreId)?.name }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="支付状态" min-width="80" prop="payStatus">
        <template #default="{ row }">
